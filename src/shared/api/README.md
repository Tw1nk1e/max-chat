...
`apiUrl` уникален для каждого инстанса (например `https://7103.api.green-api.com`), общего дефолта нет: значение видно в личном кабинете. Шаблон запроса одинаковый для всех методов:
...
**Открытый вопрос для этапа 5**: в клиенте (этот этап) реализованы только `sendMessage`, `receiveNotification`, `deleteNotification`, `getStateInstance` - без `CheckAccount`. Создание чата по номеру телефона потребует решить, отправлять ли сразу по `{{phone}}@c.us` (проще, но `chatId` в переписке может "разъехаться" с тем, что придёт в вебхуке) или сначала резолвить `chatId` через `CheckAccount` (надёжнее, но лишний запрос). Нужно решить на этапе 5.
